...
set(_exported_dependencies "rclcpp;rclcpp_lifecycle;message_filters;tf2_geometry_msgs;geometry_msgs;nav_msgs;sensor_msgs;std_srvs;tf2_ros;tf2;nav2_util;nav2_msgs")
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(nav2_amcl_INCLUDE_DIRS "${_include_dirs}")
          endif()
